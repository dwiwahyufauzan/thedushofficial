<script lang="ts">
  // ============================================================
  // SocialLinks.svelte — Social media link icons
  // Digunakan di Footer dan halaman Contact
  // ============================================================
  ...
  // Platform definitions dengan SVG icon path
  const platforms = [
    {
      key: 'instagram' as const,
      label: 'Instagram',
      // SVG path untuk Instagram
  ...
  // Filter hanya platform yang ada linknya
  const activeLinks = platforms.filter(
    (p) => links[p.key] && links[p.key] !== ''
  );
</script>

<ul
  class="flex {direction === 'column' ? 'flex-col' : 'flex-row'} gap-4 items-center list-none p-0 m-0 {className}"
...
>
  {#each activeLinks as platform}
    <li class="list-none">
      <a
        ...
        aria-label="{platform.label} — The Dush Official"
        class="text-[#a0a0a0] hover:text-[#f5f5f5] transition-all duration-200 hover:scale-110 flex items-center justify-center"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          fill="currentColor"
          class={sizeClasses[size as 'sm' | 'md' | 'lg']}
          aria-hidden="true"
        >
  ...
  <!-- Fallback jika tidak ada link aktif -->
  {#if activeLinks.length === 0}
    <li class="list-none">
      <p class="text-[#606060] text-xs uppercase tracking-widest">
        Coming soon
      </p>
    </li>
  {/if}
